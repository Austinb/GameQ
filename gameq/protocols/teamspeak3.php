...
class GameQ_Protocols_Teamspeak3 extends GameQ_Protocols
{
	...
	protected $normalize = array(
		...
		'general' => array(
			...
			'hostname' => array('virtualserver_name'),
			'password' => array('virtualserver_flag_password'),
			'numplayers' => array('virtualserver_clientsonline'),
			'maxplayers' => array('virtualserver_maxclients'),
		...
		foreach($data AS $key => $value)
		{
			...
			switch ($key)
			{
				case 'virtualserver_name':
					$result->add('hostname', $value);
					break;
				case 'virtualserver_flag_password':
					$result->add('password', (bool) $value);
					break;
				case 'virtualserver_maxclients':
					$result->add('max_players', $value);
					break;
			}
		}
